...
def build_agent(llm):
    ...
    class HRState(TypedDict):
        ...
        coaching_guide_report: str   # stored as JSON string to match tool schema
        duration:              str
    ...
    def python_repl(code: str) -> str:
        ...
        try:
            ...
            # Patch matplotlib to use non-interactive Agg backend (no display needed)
            safe_code = (
                "import matplotlib\nmatplotlib.use(\'Agg\')\n"
                "import matplotlib.pyplot as plt\n"
                "import warnings\nwarnings.filterwarnings(\'ignore\')\n"
                + code
                # Replace plt.show() with savefig to buffer (silent)
                .replace("plt.show()", "plt.savefig('/tmp/chart.png', bbox_inches=\'tight\'); plt.close(\'all\')")
            )
...
Now proceed: call python_repl to visualize scores, then call pdf_tool_node to generate the final report."""
            return {
                ...
                "coaching_guide_report": json.dumps(data.get("coaching_guide_report", {})),
...
- NEVER give feedback without receiving the transcript first.
    ...
    def tool_node_fn(state: HRState):
        ...
        for tool_call in last_message.tool_calls:
            ...
            # Deep copy args so we can safely mutate
            tool_args = dict(tool_call["args"])

            # FIX: coaching_guide_report must always be a JSON string, never a dict
            if tool_name == "pdf_tool_node":
                cgr = tool_args.get("coaching_guide_report", "{}")
                if isinstance(cgr, dict):
                    tool_args["coaching_guide_report"] = json.dumps(cgr)
                elif not isinstance(cgr, str):
                    tool_args["coaching_guide_report"] = str(cgr)
                # Ensure list fields are always lists
                for list_field in ["tone_flags", "keywords", "strengths", "improvement_areas"]:
                    if not isinstance(tool_args.get(list_field), list):
                        tool_args[list_field] = []
                # Ensure numeric fields are floats
                for float_field in ["sentiment_score", "empathy_score"]:
                    try:
                        tool_args[float_field] = float(tool_args.get(float_field, 0.5))
                    except (TypeError, ValueError):
                        tool_args[float_field] = 0.5
    ...
    if send_clicked and user_input.strip():
        ...
        with st.spinner("🎯 Analyzing..."):
            try:
                ...
                for key in ["sentiment_score", "empathy_score", "category", "tone_flags", "keywords", "strengths", "improvement_areas"]:
                    if result.get(key):
                        if st.session_state.analysis_data is None:
                            st.session_state.analysis_data = {}
                        st.session_state.analysis_data[key] = result[key]
                # Parse coaching_guide_report back to dict for display
                if result.get("coaching_guide_report"):
                    if st.session_state.analysis_data is None:
                        st.session_state.analysis_data = {}
                    raw_guide = result["coaching_guide_report"]
                    if isinstance(raw_guide, str):
                        try:
                            st.session_state.analysis_data["coaching_guide_report"] = json.loads(raw_guide)
                        except:
                            st.session_state.analysis_data["coaching_guide_report"] = {}
                    else:
                        st.session_state.analysis_data["coaching_guide_report"] = raw_guide
