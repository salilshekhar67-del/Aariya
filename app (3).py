...
- Never give feedback without the transcript"""
    ...
    def tool_node_fn(state: HRState):
        ...
        for tool_call in last_message.tool_calls:
            ...
            # Normalize list fields — LLM sometimes sends dicts instead of strings
            # e.g. future_plan: [{"title":"..","author":".."}] -> ["Title by Author"]
            for list_field in ["future_plan", "improvements", "suggest_actions",
                               "tone_flags", "keywords", "strengths", "improvement_areas"]:
                raw = tool_args.get(list_field)
                if isinstance(raw, list):
                    ...
                    for item in raw:
                        if isinstance(item, dict):
                            ...
                            normalized.append(f"{title} by {author}".strip(" by") if author else str(title))
                        else:
                            normalized.append(str(item))
                    tool_args[list_field] = normalized
